...
const ItemGrid = ({ cat }: { cat: string }) => {
  //iz itema uzimaj glavnu kategoriju ako cak i ne podkategoriju vidit cemo
  const items = [
    {
      id: 1,
    ...
    <section id="item-grid" className="mb-20">
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-y-12 gap-x-4">
        {items.map((item) => (
          <div key={item.id}>
            <Link
              href={`/kategorije/${item.itemCat}/${cat}/${item.title}`}
